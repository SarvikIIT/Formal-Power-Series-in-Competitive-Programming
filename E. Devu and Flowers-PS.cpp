...
inline mint nCr_large(long long n, int r){
    if(r < 0 || n < r) return 0;
    mint res = 1;
    rep(i,1,r+1){
        res *= (n - r + i);
...
void solve() {
    ...
    /*
                Observations:
        1) We can write the options as (1+x+x^2...x^fi) for each flower type and multiply them to get the final generating function.
        2) Then we can use it to find the value of coefficient of x^s in the final generating function.
        Solving a bit we get [x^s] (x^(f1+1)-1)/(x-1) * (x^(f2+1)-1)/(x-1) * ... * (x^(fn+1)-1)/(x-1)
        => [x^s] (x^(f1+1)-1)(x^(f2+1)-1)...(x^(fn+1)-1) / (x-1)^n
        3) Now expanding denominator we get:
        [x^s] (x^(f1+1)-1)(x^(f2+1)-1)...(x^(fn+1)-1) * sum(k=0 to inf) C(n-1+k, n-1) x^k:
        This we can easily compute using either FFT or NTT. for the numerator and then enumrate over all values of k to get the answer.
    */
    vector<mint> numerator(s+1);
    numerator[0] = 1;
    each(fi,a) {
        vi curr_poly(fi+2,0);
        curr_poly[0] = 1;
        curr_poly[fi+1] = -1;
        vi numerator_ll(numerator.size());
        rep(i,0,numerator.size()) numerator_ll[i] = numerator[i].value;
        vi multiplied = multiply(numerator_ll,curr_poly);
        vector<mint> new_numerator(s+1);
        rep(i,0,min((int)multiplied.size(),s+1)) {
            new_numerator[i] = multiplied[i];
        }
        numerator = new_numerator;
    }
    mint ans = 0;
    rep(k,0,s+1) {
        mint ways = nCr(n-1+k,n-1);
        ans += numerator[s-k] * ways;
    }
    cout<<ans<<endl;
    ...
    rep(mask,0,lim){
        int sub = 0;
        rep(i,0,n) if(mask&(1LL<<i)) sub +=a[i]+1;
        int rem =s-sub;
        if(rem<0) continue;
        mint ways = nCr_large(rem+n-1,n-1);
        if(__builtin_popcountll(mask)&1)ans-=ways;
        else ans+=ways;
    }
    cout<<ans<<endl;
}
