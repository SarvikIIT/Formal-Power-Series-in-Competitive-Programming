...
void solve() {
    cin>>n>>m>>s;
    // We need coeff of x^S in (1 - x^{M+1})^N * (1 - x)^{-N}
    // (1 - x^{M+1})^N contributes: (-1)^k * nCk * x^{k(M+1)}
    // (1 - x)^{-N}    contributes: nCr(N + rem - 1, N - 1) * x^{rem}
    int ans = 0;
    rep(k,0,n){
        int p = k*(m+1);
        if(p>s)break;
        int ways = nCr(n+s-p-1,n-1);
        int term = (nCr(n,k)*ways)%MOD;
        if(k&1){
