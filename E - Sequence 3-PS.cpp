...
void solve() {
    cin>>n>>m;
    /*
    we can take a number i as : 1 + x + x^2/2! ... x^i/i!:
    thus the product of all values from i 1 to m should have coefficient of x^n:
    */
    int ans = fact[n];
    vi poly = {1};
    rep(i,1,m+1){
        ...
        poly=mul(poly,term);
        while(sz(poly)>n+1)poly.pop_back();
    }
    if(sz(poly)<=n){
        cout<<0<<endl;
    }
    else{
        cout<<(poly[n]*fact[n])%MOD<<endl;
    }
