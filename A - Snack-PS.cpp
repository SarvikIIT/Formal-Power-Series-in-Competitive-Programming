...
void solve() {
    ...
    int limit = n;
    /*
        We can factorise the polynomial and simplify it to the coefficient of x^(n-d) in (1+x^2)^d*(1+x^3)^d:
        now the answer is then (dCr)*(dCk) for all r and k that satisfy the value :
        2r+3k = N-D:
    */
    int x0,y0;
    int val = gcd1(2,3,x0,y0);
    if((d-n)%val){
        cout<<0<<endl;
        return;
    }
    x0 = x0 * (d-n)/val;
    y0 = y0 * (d-n)/val;
    // We now have the  base case of values that satisfy the answer:
    // now we can enumarate all the solutions and then get the required answer:
    int k_shift = x0/3;
    x0 -= k_shift*3;
    y0 += k_shift*2;
    if(x0<0){
    ...
    while(x0<=limit){
        if(y0>=0&&y0<=limit){
            ans+= mint((int64_t)(nCr(limit,x0)*nCr(limit,y0)));
        }
        x0+=3;
        y0-=2;
        if(y0<0)break;
