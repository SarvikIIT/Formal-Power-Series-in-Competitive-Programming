    ...
    */
    ...
    void solve() {
        ...
        /*
                                                    Observations:
            1) We can solve it using two separate generating functions and use them to get the coefficient of x^n and y^m
            2) In one move we can get two cases : a) x(1+y+y^2....) or b) 1(y+y^2....): thus it solves to be (x+y)/(1-y)
            3) Now this move can be done either once or twice or thrice .... hence calling the function f(x,y):
                Ans(x,y) = (1 + f(x,y) + f(x,y)^2 + f(x,y)^3 + ....) = 1/(1-f(x,y)) = 1/(1 - (x+y)/(1-y)) = (1-y)/(1-x-2y)
            4) Now we need to find the coefficient of x^n y^m in the Ans(x,y) function:
                Solving further we get Ans(x,y) = Sigma i = 0 to infinity (x+2y)^i * (1-y)
                The two coefficients we require thus are:
                kCr 2^r x^(k-r) y^ r and - kCr 2^r x^(k-r) y^(r+1)
                ,Solving for the values we get r = m and k = n+m for first case and k = n+m-1 for second case
            5) Thus final answer becomes:
                Ans = nCr(n+m, m) * 2^m - nCr(n+m-1, m-1) * 2^(m-1)
        */
    mint ans = 0;
        ans += mint(nCr(n+m, m)) * mint(power(2, m));
        if(m >= 1)
            ans -= mint(nCr(n+m-1, m-1)) * mint(power(2, m-1));
        cout<<ans<<endl;
    }
