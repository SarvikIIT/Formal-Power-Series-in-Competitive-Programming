...
void solve() {
    ...
    /*
        Observations:
        1) no two numbers are same:
        2) we need atleast n unique vales thus m-n>=0:
        suppose we take the first number as any number clearly the common differences must be odd hence we need to add odd values n-1 times:
        thus the term becomes 1/(1-z) * (z/(1-z^2))^n-1 * 1/(1-z): the last 1/1-z is for prefix sum to find the value of all possible values till M, cute trick:
        which is now gives coefficient of M as:
        2*(j+n)Cn for odd resultant  and (j+n)Cn + (j+n-1)Cn for even resultant:
    */
    int res = m-n+1;
    int ans = 0;
    if(res&1){
